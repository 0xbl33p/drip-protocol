...
fn t11_50_execute_trade_atomic_oi_update_sign_flip() {
    ...
    let r1 = engine.execute_trade(a, b, 100, 1, size_q, 100, 0i64);
    assert!(r1.is_ok());
...
fn t11_51_execute_trade_slippage_zero_sum() {
    ...
    let size_q = POS_SCALE as i128;
    let result = engine.execute_trade(a, b, 100, 1, size_q, 100, 0i64);
    assert!(result.is_ok());
...
fn proof_property_51_withdrawal_dust_guard() {
    ...
    let result = engine.withdraw(a, 4500, DEFAULT_ORACLE, DEFAULT_SLOT, 0i64);
    assert!(result.is_err(),
        "withdrawal leaving dust capital (500 < 1000) must be rejected");

    // Withdraw leaving exactly 0 → must succeed
    let result_zero = engine.withdraw(a, 5000, DEFAULT_ORACLE, DEFAULT_SLOT, 0i64);
...
fn proof_property_31_missing_account_safety() {
    ...
    let settle_result = engine.settle_account(missing, DEFAULT_ORACLE, DEFAULT_SLOT, 0i64);
    assert!(settle_result.is_err(), "settle_account must reject missing account");

    // withdraw must reject missing account
    let withdraw_result = engine.withdraw(missing, 100, DEFAULT_ORACLE, DEFAULT_SLOT, 0i64);
    ...
    let trade_result = engine.execute_trade(missing, real, DEFAULT_ORACLE, DEFAULT_SLOT,
        POS_SCALE as i128, DEFAULT_ORACLE, 0i64);
    ...
    let liq_result = engine.liquidate_at_oracle(missing, DEFAULT_SLOT, DEFAULT_ORACLE, LiquidationPolicy::FullClose, 0i64);
...
fn proof_property_49_profit_conversion_reserve_preservation() {
    ...
    engine.keeper_crank(slot3, high_oracle, &[(a, None)], 64, 0i64).unwrap();
...
fn proof_property_52_convert_released_pnl_instruction() {
    ...
    let result = engine.convert_released_pnl(a, released_before, high_oracle, slot3, 0i64);
